...
class AvgSimpleQConfig(AlgorithmConfig):
    ...
    def __init__(self, algo_class=None):
        """Initializes a AvgSimpleQConfig instance."""
        super().__init__(algo_class=algo_class or AvgSimpleQ)
...
class AvgSimpleQ(Algorithm):
    @classmethod
    @override(Algorithm)
    def get_default_config(cls) -> AlgorithmConfig:
        return AvgSimpleQConfig()
    ...
    def training_step(self) -> ResultDict:
        ...
        # print("NEW SAMPLE BATCHES: ", new_sample_batches)
        # print("new rewards: ", new_sample_batches[0]['default_policy']['rewards'])
        ...
        for batch in new_sample_batches:
            ...
            new_rewards = np.append(new_rewards, batch['default_policy']['rewards'])

        print("NEW REWARDS MEAN: ", np.mean(new_rewards))
        lw_policy = local_worker.get_policy()
        alpha = lw_policy.config["apo_step_size"]
        lw_policy.average_reward_estimate = (1-alpha) * lw_policy.average_reward_estimate + alpha * np.mean(new_rewards)
...
class _deprecated_default_config(dict):
    def __init__(self):
        super().__init__(AvgSimpleQConfig().to_dict())
