...
def get_ppo_tf_policy(name: str, base: TFPolicyV2Type) -> TFPolicyV2Type:
    ...
    class PPOTFPolicy(
        ...
        def __init__(
            ...
            self.action_dims = reduce(lambda x,y: x*y, self.action_space.shape)
            # Initialize MixIns.
        ...
        def action_sampler_fn(self, model, input_dict, **kwargs,):
            seq_lens = kwargs.get("seq_lens")
            state_batches = kwargs.get("state_batches")
            ...
            if isinstance(model, tf.keras.Model):
                    ...
                    dist_inputs, state_out, extra_fetches = model(input_dict)
            else:
                    dist_inputs, state_out = model(input_dict, state_batches, seq_lens)

            action_dist = self.dist_class(dist_inputs, model)
            ...
            #print("ACTIONS BEFORE MASKING: ", actions)
            #print("OBS: ", input_dict[SampleBatch.OBS])

            mask_low = input_dict[SampleBatch.OBS][:,0:self.action_dims]
            mask_high = input_dict[SampleBatch.OBS][:,self.action_dims:self.action_dims*2]
            
            actions = tf.math.minimum(tf.math.maximum(actions, mask_low), mask_high)
            logp = None
            #print("MASKS: ", mask_low, mask_high)
            #print("ACTIONS AFTER MASKING: ", actions)
