"""
Discounted Average Policy Optimization (APO)
==================================
...
from ray.rllib.evaluation.postprocessing import compute_dapo_advantage_for_sample_batch
...
class DAPOConfig(PGConfig):
    """Defines a configuration class from which a DAPO Algorithm can be built.

    Example:
        >>> from ray.rllib.algorithms.apo import APOConfig
        >>> config = DAPOConfig()  # doctest: +SKIP
    ...
    Example:
        >>> from ray.rllib.algorithms.dapo import DAPOConfig
        >>> from ray import air
    ...
    def __init__(self, algo_class=None):
        """Initializes a DAPOConfig instance."""
        super().__init__(algo_class=algo_class or DAPO)
    ...
    def training(
        ...
        *,
        ...
        dapo_step_size: Optional[float] = NotProvided,
        ...
        **kwargs,
        ...
        if vf_share_layers != DEPRECATED_VALUE:
            deprecation_warning(
                ...
                new="DAPOConfig().training(model={'vf_share_layers': ...})",
                error=True,
            )
        ...
        if dapo_step_size is not NotProvided:
            self.dapo_step_size = dapo_step_size
    ...
    def validate(self) -> None:
        ...
        if self.sgd_minibatch_size > self.train_batch_size:
            raise ValueError(
                ...
                f"`train_batch_size` ({self.train_batch_size}). In DAPO, the train batch"
...
class UpdateKL:
    ...
    a `update_kl` method for this to work. This is achieved for DAPO via a
    ...
    def get_default_policy_class(
        ...
        if config["framework"] == "torch":
            ...
            if config._enable_rl_module_api:
                ...
                return DAPOTorchPolicy
        elif config["framework"] == "tf":
            from ray.rllib.algorithms.dapo.dapo_tf_policy import DAPOTF1Policy

            return DAPOTF1Policy
        else:
            if config._enable_rl_module_api:
                ...
                from ray.rllib.algorithms.dapo.dapo_tf_policy import DAPOTF2Policy

                return DAPOTF2Policy

    @ExperimentalAPI
    def training_step(self) -> ResultDict:
        if self.workers.local_worker().policy_map['default_policy'].discounted_average_reward_estimate == self.workers.local_worker().policy_map['default_policy'].config["init_average_reward_estimate"]:
            print("CREATE DAPO ENV")
        ...
        if self.workers.local_worker().policy_map['default_policy'].config["use_bias_normalization"]:
            ...
            print("STARTING STATE DAPO: ", start_price, " " , start_inv)
        ...
        print("TRAIN_BATCH DAPO: ", train_batch.policy_batches['default_policy'])

        train_batch.policy_batches['default_policy'] = compute_dapo_advantage_for_sample_batch(self.workers.local_worker().get_policy(), train_batch.policy_batches['default_policy'], self.config.lambda_, update_estimates=True, bias_regularization=bias_regularization)
       
        for k in self.workers.local_worker().policy_map.keys():
            mean_reward = train_batch.policy_batches[k]["discounted_rewards"].mean()
            mean_value = train_batch.policy_batches[k]["vf_preds"].mean()
            dapo_step_size = self.workers.local_worker().policy_map[k].config["dapo_step_size"]
            self.workers.local_worker().policy_map[k].discounted_average_reward_estimate = (1-dapo_step_size) *  self.workers.local_worker().policy_map[k].discounted_average_reward_estimate + dapo_step_size * (mean_reward + bias_regularization)
            self.workers.local_worker().policy_map[k].bias_estimate = (1-dapo_step_size) * self.workers.local_worker().policy_map[k].bias_estimate + dapo_step_size * (mean_value + bias_regularization)
        ...
        updated_average_reward = np.mean([self.workers.local_worker().policy_map[pid].discounted_average_reward_estimate for pid in policies_to_update])
        ...
        print("LOCAL WORKER DISCOUNTED AVERAGE REWARD: ", self.workers.local_worker().get_policy().discounted_average_reward_estimate)
        ...
        def update_apo_estimates(w):
            for k in w.policy_map.keys():
                ...
                # print(w)
                # print(w.policy_map[k].discounted_average_reward_estimate)
                w.policy_map[k].discounted_average_reward_estimate = updated_average_reward
...
class _deprecated_default_config(dict):
    def __init__(self):
        super().__init__(DAPOConfig().to_dict())

    @Deprecated(
        old="ray.rllib.agents.dapo.dapo::DEFAULT_CONFIG",
        new="ray.rllib.algorithms.dapo.dapo::DAPOConfig(...)",
        error=True,
    )
