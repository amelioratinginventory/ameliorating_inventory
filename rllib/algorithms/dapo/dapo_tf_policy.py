"""
TensorFlow policy class used for APO.
"""
...
from ray.rllib.evaluation.postprocessing import (
    Postprocessing,
    compute_dapo_advantage_for_sample_batch,
)
...
def get_dapo_tf_policy(name: str, base: TFPolicyV2Type) -> TFPolicyV2Type:
    """Construct a DAPOTFPolicy inheriting either dynamic or eager base policies.

    Args:
        base: Base class for this policy. DynamicTFPolicyV2 or EagerTFPolicyV2.

    Returns:
        A TF Policy to be used with DAPO.
    """
    ...
    class DAPOTFPolicy(
        ...
        def __init__(
            ...
            config = dict(ray.rllib.algorithms.dapo.dapo.DAPOConfig().to_dict(), **config)
            ...
            self.view_requirements["infos"] = ViewRequirement(SampleBatch.INFOS, shift=1)

            self.discounted_average_reward_estimate=np.mean([self.config["init_average_reward_estimate"]])
        ...
        def loss(
            ...
            if self.config["use_critic"]:
                vf_targets_bias_control = train_batch[Postprocessing.VALUE_TARGETS] - self.config["bias_factor"] * self.bias_estimate
        ...
        def stats_fn(self, train_batch: SampleBatch) -> Dict[str, TensorType]:
            return {
                ...
                #"price_impact": explained_variance(train_batch[SampleBatch.OBS][:,0], train_batch[SampleBatch.ACTIONS]),
                "average_reward_estimate": tf.cast(self.discounted_average_reward_estimate, tf.float64),
        ...
        def postprocess_trajectory(
            ...
            return compute_dapo_advantage_for_sample_batch(
                self, sample_batch, self.config["lambda"], False, other_agent_batches, episode
            )

    DAPOTFPolicy.__name__ = name
    DAPOTFPolicy.__qualname__ = name

    return DAPOTFPolicy


DAPOTF1Policy = get_dapo_tf_policy("DAPOTF1Policy", DynamicTFPolicyV2)
DAPOTF2Policy = get_dapo_tf_policy("DAPOTF2Policy", EagerTFPolicyV2)
