...
class AmelioratingInventoryEnv(gym.Env):
    ...
    def __init__(self, config=EnvContext):
        ...
        self.demandDistributions = config["demandDistributions"] if "demandDistributions" in config else [st.truncnorm(loc=10.0, scale=1.5, a=(-10.0/1.5),b=(10.0/1.5)) for _ in range(self.nProducts)]
        ...
        self.correlation_demand_salesprice = config["correlation_demand_salesprice"] if "correlation_demand_salesprice" in config else np.array([0.0 for _ in range(self.nProducts)])
        self.qualityDistribution = config["qualityDistribution"] if "qualityDistribution" in config else st.rv_discrete(values=([0],[1.0]))
        self.numQualityLevels = int(self.qualityDistribution.b + 1)
        self.correlation_price_quality = config["correlation_price_quality"] if "correlation_price_quality" in config else 0.0
        self.decay_mean = config["decay_mean"] if "decay_mean" in config else [[0.02 for _ in range(self.numAges)] for _ in range(self.numQualityLevels)]
        self.decay_cov = config["decay_cov"] if "decay_cov" in config else [[0.8 for _ in range(self.numAges)] for _ in range(self.numQualityLevels)]
        v_decay = [[(self.decay_mean[q][i]*(1-self.decay_mean[q][i]))/((self.decay_cov[q][i] * self.decay_mean[q][i])**2) - 1 for i in range(self.numAges)] for q in range(self.numQualityLevels)]
        alpha_decay = [[self.decay_mean[q][i]*v_decay[q][i] for i in range(self.numAges)] for q in range(self.numQualityLevels)]
        beta_decay = [[(1-self.decay_mean[q][i])*v_decay[q][i] for i in range(self.numAges)] for q in range(self.numQualityLevels)]
        self.decayProbabilities = {q: {i: st.beta(a=alpha_decay[q][i], b=beta_decay[q][i]) for i in range(self.numAges)} for q in range(self.numQualityLevels)}
        ...
        self.price_quality_distribution = st.multivariate_normal([0.0,0.0], [[1.0, self.correlation_price_quality],[self.correlation_price_quality, 1.0]])
        self.norm = st.norm()
        ...
        self.sales_bound = [round(self.demandDistributions[i].ppf(self.max_ppf),2) for i in range(self.nProducts)]
        ...
        self.sales_bound = [self.production_levels[p][-1] for p in range(self.nProducts)]
        print("SALES BOUND: ", self.sales_bound)
        self.production_levels_lp = {p: [round(i,2) for i in np.arange(0,self.sales_bound[p], self.production_step_size_lp)] for p in range(self.nProducts)}
        ...
        self.meanDecay = {q: {i: self.decayProbabilities[q][i].mean() for i in range(self.numAges)} for q in range(self.numQualityLevels)}
        self.meanDecay_age = {i: np.array([self.decayProbabilities[q][i].mean() for q in range(self.numQualityLevels)]) for i in range(self.numAges)}
        self.meanWeightedDecay = {i: sum(self.decayProbabilities[q][i].mean() * self.qualityDistribution.pmf(q) for q in range(self.numQualityLevels)) for i in range(self.numAges)}
        self.update_quality_expectation_price()
        self.quality_sample = self.qualityDistribution.rvs()
        ...
        if "expected_revenue" in config:
            ...
            self.expected_revenue = {p: {l: self.expected_revenue_function(p,l) for l in self.production_levels[p]} for p in range(self.nProducts)}

        if "slope" in config:
            self.slope = config["slope"]
        else:
            self.slope = {p: {l: self.slope_function(p,l) for l in self.production_levels[p]} for p in range(self.nProducts)}
        ...
        self.quality_levels = np.array([self.qualityDistribution.rvs() for _ in range(self.numAges)])
        ...
        if self.use_issuance_model:
            if not self.drl_for_production:
                self.heuristic_action = [self.heuristic_model.getVarByName("inv[0,1]").X/(1-self.meanWeightedDecay_price[0])]
                self.action_history = np.array([[self.heuristic_action[0]/self.maxInventory] for i in range(self.history_length)])
            else:
                self.heuristic_action = [self.heuristic_model.getVarByName("inv[0,1]").X/(1-self.meanWeightedDecay_price[0])] + [sum(self.heuristic_model.getVarByName(f"iss[{p},{a},0]").X for a in range(self.numAges)) for p in range(self.nProducts)]
            ...
            self.heuristic_action = [self.heuristic_model.getVarByName("inv[0,1]").X/(1-self.meanWeightedDecay_price[0])] + [sum(self.heuristic_model.getVarByName(f"iss[{p},{a},0]").X for p in range(self.nProducts)) for a in range(self.numAges)]
        ...
        self.price_history = np.array([self.priceDistribution.cdf(self.meanPrice) for i in range(self.history_length)])
        self.inventory_history = np.array([self.inventory_position for i in range(self.history_length)])
        self.quality_history = np.array([int(self.qualityDistribution.median())/(self.numQualityLevels-1) for _ in range(self.history_length)])
        ...
        if self.action_space_design == "box_continuous":
            ...
            if self.penalty_structure > 0:
                original_space = spaces.Dict({
                    ...
                    "quality": spaces.Box(low=np.full((self.numAges,),0.0), high=np.full((self.numAges,),1.0), shape=(self.numAges,)),
                    ...
                    "quality_history": spaces.Box(low=np.full((self.history_length,),0.0),high=np.full((self.history_length,),1.0),shape=(self.history_length,)),
                })
            else:
                original_space = spaces.Dict({
                    "price": spaces.Box(low=0.0, high=1.0, shape = (1,)),
    ...
    def quality_dist_given_price(self, p):
        quantile = min(1-1e-12, max(1e-12, self.priceDistribution.cdf(p)))
        p_norm = self.norm.ppf(quantile)
        relative_probs = np.array([sigr.quad(lambda q: self.price_quality_distribution.pdf([p_norm,q]), self.norm.ppf(self.qualityDistribution.cdf(i-1)), self.norm.ppf(self.qualityDistribution.cdf(i)))[0] for i in range(self.numQualityLevels)])
        ret = relative_probs/sum(relative_probs)
        assert(abs(sum(ret) - 1.0) < 1e-9)
        return ret
    
    def update_quality_expectation_price(self):
        self.q_dist_price = self.quality_dist_given_price(self.price)
        self.meanWeightedDecay_price = np.array([np.dot(self.q_dist_price, self.meanDecay_age[i]) for i in range(self.numAges)])
    ...
    def get_heuristic_action(self):
        ...
        purchasing = self.heuristic_model.getVarByName("inv[0,1]").X/(1-self.meanWeightedDecay_price[0])
    ...
    def get_structure_deviation(self, purchasing, production):
        ...
        for i in range(self.history_length):
            ...
            hist_q = np.concatenate((self.quality_levels/(self.numQualityLevels-1),self.quality_history))

            if (purchasing > hist_a and curr_p >= hist_p and not any(self.inventory_position[a] < hist_inv[a] and hist_q[a] >= hist_q[a+i+1] for a in range(self.numAges))) or (purchasing < hist_a and curr_p <= hist_p and not any(self.inventory_position[a] > hist_inv[a] and hist_q[a] <= hist_q[a+i+1] for a in range(self.numAges))):
    ...
    def update_history(self,purchasing,production):
        ...
        self.quality_history = np.insert(self.quality_history[:-1],0,self.quality_levels[-1]/(self.numQualityLevels-1))
    ...
    def  _get_obs(self):
        if self.penalty_structure > 0:
            return {"price": np.array([self.priceDistribution.cdf(self.price)]), "inventory": self.inventory_position/self.maxInventory, "quality": self.quality_levels/(self.numQualityLevels-1), "action_history":self.action_history, "price_history":self.price_history, "inventory_history":self.inventory_history/self.maxInventory, "quality_history":self.quality_history}
        else:
            return {"price": np.array([self.priceDistribution.cdf(self.price)]), "inventory": self.inventory_position/self.maxInventory, "quality": self.quality_levels/(self.numQualityLevels-1)} 
    ...
    def _get_state_from_obs(self, obs):
        ...
        self.quality_levels = np.array([int(obs["quality"][i] * (self.numQualityLevels-1)) for i in range(self.numAges)])
        if self.penalty_structure > 0:
            ...
            self.quality_history = obs["quality_history"]
    ...
    def step_continuous(self, action):
        ...
        if self.use_cdfs_for_regularization:
            decay_proportions = np.array([self.decayProbabilities[self.quality_levels[i]][i].ppf(self.cdf_buffer[self.n_steps-1][i+1]) for i in range(self.numAges)])
        elif self.use_adversarial_sampling and self.n_steps > self.adversarial_threshold:
            decay_proportions = np.array([self.decayProbabilities[self.quality_levels[i]][i].ppf(self.adversarial_buffer[self.n_steps-self.adversarial_threshold - 1][i+1]) for i in range(self.numAges)])
        else:
            decay_proportions = np.array([self.decayProbabilities[self.quality_levels[i]][i].rvs() for i in range(self.numAges)])
        ...
        cdf_vector = np.concatenate(([self.priceDistribution.cdf(self.price)],[self.decayProbabilities[self.quality_levels[i]][i].cdf(decay_proportions[i]) for i in range(self.numAges)]))
        if self.use_adversarial_sampling and self.n_steps <= self.adversarial_threshold:
            if self.n_steps == 1:
    ...
    def step_continuous_issuance_lp(self, action):
        ...
        self.quality_levels = np.concatenate(([self.quality_sample],self.quality_levels[:-1]))
        ...
        revenue = sum(revenue_product[p] for p in range(self.nProducts)) 
        ...
        if self.use_cdfs_for_regularization:
            self.price = self.priceDistribution.ppf(self.cdf_buffer[self.n_steps-1][0])
            self.quality_sample = self.qualityDistribution.ppf(self.cdf_buffer[self.n_steps-1][-1])
        elif self.use_adversarial_sampling and self.n_steps > self.adversarial_threshold:
            self.price = self.priceDistribution.ppf(self.adversarial_buffer[self.n_steps - self.adversarial_threshold-1][0])
            self.quality_sample = self.qualityDistribution.ppf(self.adversarial_buffer[self.n_steps - self.adversarial_threshold-1][-1])
        else:
            if self.priceProcess:
                self.price = self.priceProcess.sample(1, initial=self.price)[1]
                self.quality_sample = self.qualityDistribution.rvs()
            else:
                pq_sample = self.sample_price_quality()
                self.price = pq_sample[0]
                self.quality_sample = pq_sample[1]

        # print("TIME TO SAMPLE PRICE AND QUALITY: ", time.time() - start_time)
        # start_time = time.time()       
        
        self.update_quality_expectation_price() 

        # print("TIME TO UPDATE QUALITY EXPECTATION TO PRICE: ", time.time() - start_time)
        # start_time = time.time() 
        #adversarial sampling - in first half of trajectory, add adversarial samples to buffer which can be simply selected in second half
        cdf_vector = np.concatenate(([self.priceDistribution.cdf(self.price)],[self.decayProbabilities[self.quality_levels[i]][i].cdf(decay_proportions[i]) for i in range(self.numAges)],[self.qualityDistribution.cdf(self.quality_sample)]))
        if self.use_adversarial_sampling and self.n_steps <= self.adversarial_threshold:
            if self.n_steps == 1:
                self.adversarial_buffer = 1-cdf_vector
            else:
    ...
    def sample_price_quality(self, n=1):
        ret = np.array([])
        for _ in range(n):
            sample = self.price_quality_distribution.rvs()
            price = self.priceDistribution.ppf(self.norm.cdf(sample[0]))
            quality = self.qualityDistribution.ppf(self.norm.cdf(sample[1]))
            if ret.size==0:
                ret = np.append(ret, [price, quality])
            else:
                ret = np.vstack((ret, [price, quality]))
        return ret
    ...
    def update_issuance_model(self):
        ...
        self.update_quality(self.issuance_model)
        self.issuance_model.remove(self.issuance_model.getConstrByName("startp"))
        inv = self.issuance_model.getVarByName("inv[0,1]")
        self.issuance_model.addLConstr(inv == self.just_purchased * (1-self.meanWeightedDecay_price[0]), name="startp")
        for t in range(1,min(self.numAges+1, self.time_steps)):
            for p in range(self.nProducts):
                iss = self.issuance_model.getVarByName(f"iss[{p},{t-1},{t}]")
                iss.Obj = (sum((self.decaySalvage[t-1-j]*self.meanWeightedDecay_price[t-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay_price[t-1-k] for k in range(j+1)])) for j in range(t)) \
                        - self.meanPrice/(np.multiply.reduce([1-self.meanWeightedDecay_price[t-1-j] for j in range(t)])))
        if self.drl_for_production:
    ...
    def update_heuristic_model(self):
        ...
        self.update_quality(self.heuristic_model)
        self.heuristic_model.remove(self.heuristic_model.getConstrByName("startp"))
        inv = self.heuristic_model.getVarByName("inv[0,1]")
        self.heuristic_model.addLConstr(inv <= (self.maxInventory)*(1-self.meanWeightedDecay_price[0]), name="startp")
        for t in range(1,min(self.numAges+1,self.time_steps)):
            for p in range(self.nProducts):
                purchase = self.heuristic_model.getVarByName(f"iss[{p},{t-1},{t}]")
                purchase.Obj = (sum((self.decaySalvage[t-1-j]*self.meanWeightedDecay_price[t-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay_price[t-1-k] for k in range(j+1)])) for j in range(t)) \
                            - self.price/(np.multiply.reduce([1-self.meanWeightedDecay_price[t-1-j] for j in range(t)])))
        self.heuristic_model.update()
    ...
    def update_inventory_constraints(self, model):  
        ...
        for t in range(1,self.time_steps):
            for a in range(t,self.numAges):
                model.remove(model.getConstrByName(f"inv_balance{a}{t}"))
                inv = model.getVarByName(f"inv[{a},{t}]")
                inv_prev = model.getVarByName(f"inv[{a-1},{t-1}]")
                iss_prev = [model.getVarByName(f"iss[{p},{a-1},{t-1}]") for p in range(self.nProducts)]
                model.addLConstr(inv == (inv_prev - gb.quicksum(iss_prev[p] for p in range(self.nProducts)))*(1-self.meanDecay[self.quality_levels[a-t]][a]), name=f"inv_balance{a}{t}")
            if 2 <= t <= self.numAges:
                model.remove(model.getConstrByName(f"inv_balance{t-1}{t}"))
                inv = model.getVarByName(f"inv[{t-1},{t}]")
                inv_prev = model.getVarByName(f"inv[{t-2},{t-1}]")
                iss_prev = [model.getVarByName(f"iss[{p},{t-2},{t-1}]") for p in range(self.nProducts)]
                model.addLConstr(inv == (inv_prev - gb.quicksum(iss_prev[p] for p in range(self.nProducts)))*(1-self.meanWeightedDecay_price[t-1]), name=f"inv_balance{t-1}{t}")
    
    def update_quality(self, model):
        for t in range(min(self.numAges+1,self.time_steps)):
            out = model.getVarByName(f"out[{t}]")
            if t==self.numAges:
                out.Obj = (-self.outdatingCosts+sum((self.decaySalvage[self.numAges-1-j]*self.meanWeightedDecay_price[self.numAges-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay_price[self.numAges-1-k] for k in range(j+1)])) for j in range(self.numAges)) \
                        - self.meanPrice/(np.multiply.reduce([1-self.meanWeightedDecay_price[self.numAges-1-j] for j in range(self.numAges)])))
            else:    
                out.Obj = (-self.outdatingCosts+sum((self.decaySalvage[self.numAges-1-j]*self.meanDecay[self.quality_levels[-(t+1)]][self.numAges-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[-(t+1)]][self.numAges-1-k] for k in range(j+1)])) for j in range(self.numAges)) \
                        - self.meanPrice/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[-(t+1)]][self.numAges-1-j] for j in range(self.numAges)])))
            for p in range(self.nProducts):
                for a in range(t,self.numAges):
                    iss = model.getVarByName(f"iss[{p},{a},{t}]")
                    iss.Obj = (sum((self.decaySalvage[a-j]*self.meanDecay[self.quality_levels[a-t]][a-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[a-t]][a-k] for k in range(j+1)])) for j in range(a+1)) \
                            - self.meanPrice/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[a-t]][a-j] for j in range(a+1)])))
    ...
    def create_issuance_lp(self, time_horizon_redux = 0):
        ...
        self.issuance_model.addLConstr(inv[0,1] == self.just_purchased * (1-self.meanWeightedDecay_price[0]), name="startp")
        
        #maximum purchasing/inventory capacity restrictions
        for t in range(2,self.time_steps):
            self.issuance_model.addLConstr(inv[0,t] <= (self.maxInventory)*(1-self.meanWeightedDecay[0]))
        ...
        for t in range(self.time_steps):
            ...
            for p in range(self.nProducts):
                ...
                self.issuance_model.addLConstr(gb.quicksum(ff[p,l,t] * l for l in self.production_levels_lp[p]) <= gb.quicksum(iss[p,a,t] * self.evaporation_remains_per_age_class[a] for a in range(self.numAges)))
            for a in range(self.numAges):
                #inventory balance
                if t>0:
                    if a>=t:
                        self.issuance_model.addLConstr(inv[a,t] == (inv[a-1,t-1] - gb.quicksum(iss[p,a-1,t-1] for p in range(self.nProducts)))*(1-self.meanDecay[self.quality_levels[a-t]][a]), name=f"inv_balance{a}{t}")
                    elif a == t-1 and a > 0:
                        self.issuance_model.addLConstr(inv[a,t] == (inv[a-1,t-1] - gb.quicksum(iss[p,a-1,t-1] for p in range(self.nProducts)))*(1-self.meanWeightedDecay_price[a]), name=f"inv_balance{a}{t}")
                    elif a>0:
                        self.issuance_model.addLConstr(inv[a,t] == (inv[a-1,t-1] - gb.quicksum(iss[p,a-1,t-1] for p in range(self.nProducts)))*(1-self.meanWeightedDecay[a]))
        ...
        if self.drl_for_production:
            ...
            obj = gb.quicksum(ff[p,l,t] * self.expected_revenue[p][l] for p in range(self.nProducts) for l in self.production_levels_lp[p] for t in range(self.time_steps)) \
                + gb.quicksum(iss[p,a,t]*(sum((self.decaySalvage[a-j]*self.meanDecay[self.quality_levels[a-t]][a-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[a-t]][a-k] for k in range(j+1)])) for j in range(a+1)) \
                - self.meanPrice/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[a-t]][a-j] for j in range(a+1)]))) for p in range(self.nProducts) for t in range(min(self.numAges,self.time_steps)) for a in range(t,self.numAges)) \
                + gb.quicksum(iss[p,a,t]*(sum((self.decaySalvage[a-j]*self.meanWeightedDecay[a-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay[a-k] for k in range(j+1)])) for j in range(a+1)) \
                - self.meanPrice/(np.multiply.reduce([1-self.meanWeightedDecay[a-j] for j in range(a+1)]))) for p in range(self.nProducts) for t in range(2,self.time_steps) for a in range(t-1)) \
                + gb.quicksum(out[t]*(-self.outdatingCosts+sum((self.decaySalvage[self.numAges-1-j]*self.meanDecay[self.quality_levels[-(t+1)]][self.numAges-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[-(t+1)]][self.numAges-1-k] for k in range(j+1)])) for j in range(self.numAges)) \
                - self.priceDistribution.mean()/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[-(t+1)]][self.numAges-1-j] for j in range(self.numAges)]))) for t in range(min(self.numAges,self.time_steps))) \
                + gb.quicksum(out[t]*(-self.outdatingCosts+sum((self.decaySalvage[self.numAges-1-j]*self.meanWeightedDecay[self.numAges-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay[self.numAges-1-k] for k in range(j+1)])) for j in range(self.numAges)) \
                - self.priceDistribution.mean()/(np.multiply.reduce([1-self.meanWeightedDecay[self.numAges-1-j] for j in range(self.numAges)]))) for t in range(self.numAges,self.time_steps)) \
                + gb.quicksum(out[t]*(-self.outdatingCosts+sum((self.decaySalvage[self.numAges-1-j]*self.meanWeightedDecay_price[self.numAges-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay_price[self.numAges-1-k] for k in range(j+1)])) for j in range(self.numAges)) \
                - self.priceDistribution.mean()/(np.multiply.reduce([1-self.meanWeightedDecay_price[self.numAges-1-j] for j in range(self.numAges)]))) for t in range(self.numAges, min(self.numAges+1,self.time_steps))) \
                - gb.quicksum(prod_violation[p] * self.brandContributions[p] * self.time_steps for p in self.products_using_drl) \
                + gb.quicksum(iss[p,a,a+1]*(sum((self.decaySalvage[a-j]*self.meanWeightedDecay_price[a-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay_price[a-k] for k in range(j+1)])) for j in range(a+1)) \
                - self.meanPrice/(np.multiply.reduce([1-self.meanWeightedDecay_price[a-j] for j in range(a+1)]))) for p in range(self.nProducts) for a in range(min(self.numAges,self.time_steps-1)))
        else:
            obj = gb.quicksum(ff[p,l,t] * self.expected_revenue[p][l] for p in range(self.nProducts) for l in self.production_levels_lp[p] for t in range(self.time_steps)) \
                + gb.quicksum(iss[p,a,t]*(sum((self.decaySalvage[a-j]*self.meanDecay[self.quality_levels[a-t]][a-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[a-t]][a-k] for k in range(j+1)])) for j in range(a+1)) \
                - self.meanPrice/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[a-t]][a-j] for j in range(a+1)]))) for p in range(self.nProducts) for t in range(min(self.numAges,self.time_steps)) for a in range(t,self.numAges)) \
                ...
                - self.priceDistribution.mean()/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[-(t+1)]][self.numAges-1-j] for j in range(self.numAges)]))) for t in range(min(self.numAges,self.time_steps))) \
                + gb.quicksum(out[t]*(-self.outdatingCosts+sum((self.decaySalvage[self.numAges-1-j]*self.meanWeightedDecay[self.numAges-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay[self.numAges-1-k] for k in range(j+1)])) for j in range(self.numAges)) \
                - self.priceDistribution.mean()/(np.multiply.reduce([1-self.meanWeightedDecay[self.numAges-1-j] for j in range(self.numAges)]))) for t in range(self.numAges+1,self.time_steps)) \
                + gb.quicksum(out[t]*(-self.outdatingCosts+sum((self.decaySalvage[self.numAges-1-j]*self.meanWeightedDecay_price[self.numAges-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay_price[self.numAges-1-k] for k in range(j+1)])) for j in range(self.numAges)) \
                - self.priceDistribution.mean()/(np.multiply.reduce([1-self.meanWeightedDecay_price[self.numAges-1-j] for j in range(self.numAges)]))) for t in range(self.numAges, min(self.numAges+1,self.time_steps))) \
                + gb.quicksum(iss[p,a,a+1]*(sum((self.decaySalvage[a-j]*self.meanWeightedDecay_price[a-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay_price[a-k] for k in range(j+1)])) for j in range(a+1)) \
                - self.meanPrice/(np.multiply.reduce([1-self.meanWeightedDecay_price[a-j] for j in range(a+1)]))) for p in range(self.nProducts) for a in range(self.time_steps-1))
        
        self.issuance_model.setObjective(obj, GRB.MAXIMIZE)

        #self.issuance_model.write("model.lp")
    ...
    def create_heuristic_lp(self, time_horizon_redux = 0):
        ...
        ff = self.heuristic_model.addVars(tuplelist_ff, lb=[0.0 for _ in range(len(tuplelist_ff))], ub=[1.0 for _ in range(len(tuplelist_ff))], name="ff")
        ...
        #maximum purchasing/inventory capacity restrictions
        self.heuristic_model.addLConstr(inv[0,1] <= (self.maxInventory)*(1-self.meanWeightedDecay_price[0]), name="startp")
        for t in range(2,self.time_steps):
            self.heuristic_model.addLConstr(inv[0,t] <= (self.maxInventory)*(1-self.meanWeightedDecay[0]))
        ...
        for t in range(self.time_steps):
            ...
            for p in range(self.nProducts):
                ...
                #relate production to issuance volumes
                self.heuristic_model.addLConstr(gb.quicksum(ff[p,l,t] * l for l in self.production_levels_lp[p]) <= gb.quicksum(iss[p,a,t] * self.evaporation_remains_per_age_class[a] for a in range(self.numAges)))
            for a in range(self.numAges):
                #inventory balance
                if t > 0:
                    if a >= t:
                        self.heuristic_model.addLConstr(inv[a,t] == (inv[a-1,t-1] - gb.quicksum(iss[p,a-1,t-1] for p in range(self.nProducts)))*(1-self.meanDecay[self.quality_levels[a-t]][a]), name=f"inv_balance{a}{t}")
                    elif a > 0 and a == t-1:
                        self.heuristic_model.addLConstr(inv[a,t] == (inv[a-1,t-1] - gb.quicksum(iss[p,a-1,t-1] for p in range(self.nProducts)))*(1-self.meanWeightedDecay_price[a]), name=f"inv_balance{a}{t}")
                    elif a > 0:
                        self.heuristic_model.addLConstr(inv[a,t] == (inv[a-1,t-1] - gb.quicksum(iss[p,a-1,t-1] for p in range(self.nProducts)))*(1-self.meanWeightedDecay[a]))
        ...
        obj = gb.quicksum(ff[p,l,t] * self.expected_revenue[p][l] for p in range(self.nProducts) for l in self.production_levels_lp[p] for t in range(self.time_steps)) \
              ...
              - self.meanPrice/(np.multiply.reduce([1-self.meanWeightedDecay[a-j] for j in range(a+1)]))) for p in range(self.nProducts) for t in range(2,self.time_steps) for a in range(t-1)) \
              + gb.quicksum(out[t]*(-self.outdatingCosts+sum((self.decaySalvage[self.numAges-1-j]*self.meanDecay[self.quality_levels[-(t+1)]][self.numAges-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[-(t+1)]][self.numAges-1-k] for k in range(j+1)])) for j in range(self.numAges)) \
              - self.meanPrice/(np.multiply.reduce([1-self.meanDecay[self.quality_levels[-(t+1)]][self.numAges-1-j] for j in range(self.numAges)]))) for t in range(min(self.numAges,self.time_steps))) \
              + gb.quicksum(out[t]*(-self.outdatingCosts+sum((self.decaySalvage[self.numAges-1-j]*self.meanWeightedDecay[self.numAges-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay[self.numAges-1-k] for k in range(j+1)])) for j in range(self.numAges)) \
              - self.meanPrice/(np.multiply.reduce([1-self.meanWeightedDecay[self.numAges-1-j] for j in range(self.numAges)]))) for t in range(self.numAges+1,self.time_steps)) \
              + gb.quicksum(out[t]*(-self.outdatingCosts+sum((self.decaySalvage[self.numAges-1-j]*self.meanWeightedDecay_price[self.numAges-1-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay_price[self.numAges-1-k] for k in range(j+1)])) for j in range(self.numAges)) \
              - self.meanPrice/(np.multiply.reduce([1-self.meanWeightedDecay_price[self.numAges-1-j] for j in range(self.numAges)]))) for t in range(self.numAges,min(self.numAges+1,self.time_steps))) \
              + gb.quicksum(iss[p,a,a+1]*(sum((self.decaySalvage[a-j]*self.meanWeightedDecay_price[a-j]-self.holdingCosts)/(np.multiply.reduce([1-self.meanWeightedDecay_price[a-k] for k in range(j+1)])) for j in range(a+1)) \
              - self.price/(np.multiply.reduce([1-self.meanWeightedDecay_price[a-j] for j in range(a+1)]))) for p in range(self.nProducts) for a in range(self.time_steps-1))
    ...
    def create_allocation_lp(self):
        ...
        tuplelist_ff = [(p,l) for p in range(self.nProducts) for l in self.production_levels_lp[p]]
        ...
        for p in range(self.nProducts):
            ...
            self.allocation_model.addLConstr(gb.quicksum(ff[p,l] * l for l in self.production_levels_lp[p]) <= gb.quicksum(iss[p,a] * self.evaporation_remains_per_age_class[a] for a in range(self.numAges)))
            #no blending
            if not self.allowBlending:
        ...
        obj = gb.quicksum(ff[p,l] * self.expected_revenue[p][l] for p in range(self.nProducts) for l in self.production_levels_lp[p])
    ...
    def simulate_steps(self, nsteps=1, policy=None, plot=False, warm_up=100):
        ...
        while(nsteps > 0):
            ...
            purchasing_quality = np.append(purchasing_quality, self.quality_levels[0])
    ...
    def render_step(self, nsteps=1, policy=None):
        ...
        while(nsteps > 0):
            ...
            nsteps-=1
#--------------------------------------#
def upper_bound(env:AmelioratingInventoryEnv, discr_step=0.05):
    ...
    tangent_points = {p: [round(i,ndigits=2) for i in np.arange(env.production_levels[p][0],env.production_levels[p][-1]+env.production_step_size,discr_step)] for p in range(env.nProducts)}
    slope_tangent_points = {p: [env.slope[p][l] for l in tangent_points[p]] for p in range(env.nProducts)}
    expected_revenue_tangent_points = {p: {l: env.expected_revenue[p][l] for l in tangent_points[p]} for p in range(env.nProducts)} 
    ...
    price_levels_discretized = [round(i,2) for i in np.arange(env.priceDistribution.ppf(0.0), env.priceDistribution.ppf(1.0), discr_step)]
    quality_given_price = {}
    for p in price_levels_discretized+[round(env.priceDistribution.ppf(1.0),2)]:
        quality_given_price[p] = env.quality_dist_given_price(p)
        if p%20.0 == 0.0:
            print(f"quality distribution for price level {p} computed")
    ...
    quality_check = [sum(quality_given_price[l][q]*price_probabilities_discretized[l] for l in price_levels_discretized) for q in range(env.numQualityLevels)]
    print("quality check: ", quality_check, " ", [env.qualityDistribution.pmf(q) for q in range(env.numQualityLevels)])
    print("Preprocessing for upper bound model completed!")
    ...
    p_to_inv = upper_bound_model.addVars(price_levels_discretized, env.numQualityLevels, name="p_to_inv", lb=0.0, ub=env.maxInventory)
    inv = upper_bound_model.addVars(env.numAges, env.numQualityLevels, name="inv", lb=0.0, ub=env.maxInventory)
    iss = upper_bound_model.addVars(env.nProducts, env.numAges, env.numQualityLevels, name="iss", lb=0.0, ub=env.maxInventory)
    out = upper_bound_model.addVar(name="out", lb=0.0, ub=env.maxInventory)

    print("Upper Bound Model: Variable Creation Done!")

    #OBJECTIVE
    obj = gb.quicksum(gb.quicksum(ff[p,l] * expected_revenue_break_points[p][l] for l in break_points[p]) for p in range(env.nProducts)) - gb.quicksum(purchasing[p]*p*price_probabilities_discretized[p] for p in price_levels_discretized) + gb.quicksum(inv[a,q]*(env.decaySalvage[a]*env.meanDecay[q][a]-env.holdingCosts)/(1-env.meanDecay[q][a]) for q in range(env.numQualityLevels) for a in range(env.numAges)) - out*env.outdatingCosts
    ...
    for a in range(1,env.numAges):
        for q in range(env.numQualityLevels):
            upper_bound_model.addLConstr(inv[a,q] == (inv[a-1,q]-gb.quicksum(iss[p,a-1,q] for p in range(env.nProducts)))*(1-env.meanDecay[q][a]))
            upper_bound_model.addLConstr(gb.quicksum(iss[p,a,q] for p in range(env.nProducts)) <= inv[a,q])
    #inventory in first age class is determined by purchasing behavior
    for l in price_levels_discretized:
        for q in range(env.numQualityLevels):
            upper_bound_model.addLConstr(p_to_inv[l,q] >= min(quality_given_price[round(l+discr_step,2)][q],quality_given_price[l][q]) * purchasing[l])
            upper_bound_model.addLConstr(p_to_inv[l,q] <= max(quality_given_price[round(l+discr_step,2)][q],quality_given_price[l][q]) * purchasing[l])
        upper_bound_model.addLConstr(gb.quicksum(p_to_inv[l,q] for q in range(env.numQualityLevels)) == purchasing[l])

    for q in range(env.numQualityLevels):
        upper_bound_model.addLConstr(inv[0,q] == gb.quicksum(p_to_inv[l,q] * price_probabilities_discretized[l] for l in price_levels_discretized))
    
    upper_bound_model.addLConstr(out == gb.quicksum((inv[env.numAges-1,q] - gb.quicksum(iss[p,env.numAges-1,q] for p in range(env.nProducts))) for q in range(env.numQualityLevels)))
    ...
    for p in range(env.nProducts):
        ...
        upper_bound_model.addLConstr(gb.quicksum(ff[p,l]*l for l in break_points[p]) <= gb.quicksum(iss[p,a,q] * env.evaporation_remains_per_age_class[a] for a in range(env.numAges) for q in range(env.numQualityLevels)))
        #target ages need to be respected
        upper_bound_model.addLConstr(gb.quicksum(iss[p,a,q] * a * env.evaporation_remains_per_age_class[a] for a in range(env.numAges) for q in range(env.numQualityLevels)) >= env.targetAges[p] * gb.quicksum(iss[p,a,q] * env.evaporation_remains_per_age_class[a] for a in range(env.numAges) for q in range(env.numQualityLevels)))
        #prohibit blending
        if not env.allowBlending:
            upper_bound_model.addLConstr(gb.quicksum(iss[p,a,q] for a in range(env.targetAges[p]-1) for q in range(env.numQualityLevels)) <= 0)
        elif env.blendingRange is not None:
            upper_bound_model.addLConstr(gb.quicksum(iss[p,a,q] for q in range(env.numQualityLevels) for a in range(env.numAges) if a not in range(env.targetAges[p]-env.blendingRange,env.targetAges[p]+env.blendingRange+1)) <= 0)

    print("Upper Bound Model: Product Constraints Added!")

    upper_bound_model.optimize()
    opt_inv = [sum(upper_bound_model.getVarByName(f"inv[{i},{q}]").X for q in range(env.numQualityLevels)) for i in range(env.numAges)]
    print("OPTIMAL INVENTORY STRUCTURE: ", opt_inv)
    opt_inv_q = {q: [upper_bound_model.getVarByName(f"inv[{i},{q}]").X for i in range(env.numAges)] for q in range(env.numQualityLevels)}
    print("OPTIMAL INVENTORY QUALITY: ", opt_inv_q)
    opt_iss_age = [sum(upper_bound_model.getVarByName(f"iss[{p},{a},{q}]").X for q in range(env.numQualityLevels) for p in range(env.nProducts)) for a in range(env.numAges)]
    print("OPTIMAL ISSUANCE VOLUMES: ", opt_iss_age)
    opt_iss_q = {q: [sum(upper_bound_model.getVarByName(f"iss[{p},{a},{q}]").X for p in range(env.nProducts)) for a in range(env.numAges)] for q in range(env.numQualityLevels)}
    print("OPTIMAL ISSUANCE QUALITY: ", opt_iss_q)
    print("OPTIMAL PRODUCTION VOLUMES BEFORE EVAPORATION: ", [sum(upper_bound_model.getVarByName(f"iss[{p},{a},{q}]").X for q in range(env.numQualityLevels) for a in range(env.numAges)) for p in range(env.nProducts)])
    opt_prod = [sum(upper_bound_model.getVarByName(f"iss[{p},{a},{q}]").X * env.evaporation_remains_per_age_class[a] for a in range(env.numAges) for q in range(env.numQualityLevels)) for p in range(env.nProducts)]
    ...
    print("UPPER BOUND: ", upper_bound_model.ObjVal)
    ...
    opt_rev = sum(upper_bound_model.getVarByName(f"ff[{p},{l}]").X * expected_revenue_break_points[p][l] for p in range(env.nProducts) for l in break_points[p])
    print("OPTIMAL EXPECTED REVENUES: ", opt_rev)
    opt_cost_h = sum(upper_bound_model.getVarByName(f"inv[{a},{q}]").X * env.holdingCosts/(1-env.meanDecay[q][a]) for q in range(env.numQualityLevels) for a in range(env.numAges))
    print("OPTIMAL HOLDING COSTS: ", opt_cost_h)
    opt_rev_salvage = sum(upper_bound_model.getVarByName(f"inv[{a},{q}]").X * (env.decaySalvage[a]*env.meanDecay[q][a])/(1-env.meanDecay[q][a]) for q in range(env.numQualityLevels) for a in range(env.numAges))
    print("OPTIMAL SALVAGE DECAY: ", opt_rev_salvage)

    res_dict = {"max_reward": upper_bound_model.ObjVal, "inventory_position": opt_inv, "inventory_quality": opt_inv_q, "issuance_quality": opt_iss_q, "production": opt_prod, "purchasing": opt_purchasing, "purchasing_cost": opt_cost_p, "holding_cost": opt_cost_h, "expected_revenues": opt_rev, "decay_salvage": opt_rev_salvage}
