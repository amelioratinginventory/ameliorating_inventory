...
class AmelioratingInventoryEnv(gym.Env):
    ...
    def __init__(self, config=EnvContext):
        ...
        self.demand_mean = {p: self.demandDistributions[p].mean() for p in self.products}
        self.demand_std = {p: self.demandDistributions[p].std() for p in self.products}
        self.salesprice_mean = {p: self.salesPriceDistributions[p].mean() for p in self.products}
        self.salesprice_std = {p: self.salesPriceDistributions[p].std() for p in self.products}
        ...
        self.use_newsvendor = config["use_newsvendor"] if "use_newsvendor" in config else False
        ...
        self.acc_c_target = {p: sum((-self.decaySalvage[self.targetAges[p]-j]*self.meanDecay[self.targetAges[p]-j]+self.holdingCosts)/(np.multiply.reduce([1-self.meanDecay[self.targetAges[p]-k] for k in range(j+1)])) for j in range(self.targetAges[p]+1)) for p in self.products}
        self.acc_inv_target = {p: 1/np.multiply.reduce([1-self.meanDecay[self.targetAges[p]-k] for k in range(self.targetAges[p]+1)]) for p in self.products}
        ...
        #determine production targets for order-up-to heuristic
        #self.production_targets = {p: self.get_order_up_to_production_targets(p) for p in self.products}
        # self.production_targets_price = config["production_targets"] if "production_targets" in config else {price: [[0] for p in self.products] for price in np.arange(0.0,1.01,0.01)}# {price: [[self.get_order_up_to_production_targets(p,self.priceDistribution.ppf(price))] for p in self.products] for price in np.arange(0.0,1.01,0.01)}
        # self.production_targets = {p: np.mean([self.production_targets_price[price][p] for price in self.production_targets_price]) for p in self.products}
        # self.price_levels_sorted = sorted(self.production_targets_price.keys())
        # self.newsvendor_prod = config["newsvendor_prod"] if "newsvendor_prod" in config else None
        # self.inventory_levels_sorted = sorted(self.newsvendor_prod[0].keys()) if self.newsvendor_prod is not None else None

        self.use_order_up_to = config["use_order_up_to"] if "use_order_up_to" in config else False
        self.use_volume_target = config["use_volume_target"] if "use_volume_target" in config else False
        # order_up_to_total = 0
        # order_up_to_young = 0
        # for a in self.ages:
        #     for p in self.products:
        #         if a < self.targetAges[p]:
        #             order_up_to_total += self.production_targets[p] * (self.acc_inv_target[p]/self.evaporation_remains_per_age_class[self.targetAges[p]]) *  np.prod([1-self.meanDecay[j] for j in range(a+1)]) 
        #         if a < self.targetAges[0]:
        #             order_up_to_young += self.production_targets[p]* (self.acc_inv_target[p]/self.evaporation_remains_per_age_class[self.targetAges[p]]) * np.prod([1-self.meanDecay[j] for j in range(a+1)])
        # order_up_to_total += sum(self.production_targets[p] * (self.acc_inv_target[p]/self.evaporation_remains_per_age_class[self.targetAges[p]]) for p in self.products)
        # order_up_to_young += sum(self.production_targets[p] * (self.acc_inv_target[p]/self.evaporation_remains_per_age_class[self.targetAges[p]]) for p in self.products)
        # self.order_up_to_level = config["order_up_to"] if "order_up_to" in config else [order_up_to_total, order_up_to_young]
        # print("ORDER UP TO LEVEL: ", self.order_up_to_level)        


        # print("PRODUCTION TARGETS: ", self.production_targets)

        #initialize heuristic LPs   
        self.create_heuristic_lp()
        # self.create_newsvendor_lp()
        # self.create_production_target_lp()
    ...
    def step_continuous(self, action):
        ...
        return observation, norm_reward, False, self.n_steps == self.max_horizon or (self.use_adversarial_sampling and self.n_steps == self.adversarial_threshold), {"revenue":revenue, "purchasing_cost": purchasing_cost, "holding_cost":holding_cost, "decay_salvage":decay_salvage, "outdating_cost": outdating_cost, "purchasing":purchasing, "production":production, "issuance":[sum(issuance_product[p][a] for p in self.products) for a in self.ages], "demand": demand, "outdating": outdating, "overproduction": leftovers, "inventory": self.inventory_position, "sales": sales, "price":prev_price, "decay_proportions":decay_proportions, "cdf_vector":cdf_vector, "env_time": env_time, "lp_time": lp_time}
    ...
    def step_continuous_issuance_lp(self, action):
        ...
        elif self.use_newsvendor:
            purchasing = self.get_newsvendor_purchasing()
            if self.use_volume_target:
                issuance_product = self.get_production_from_target()
            else:
                issuance_product = self.get_production_from_newsvendor()
        elif self.use_order_up_to:
            #use lp for issuance
            if self.use_volume_target:
                issuance_product = self.get_production_from_target()
            else:
                issuance_product = self.get_production_from_newsvendor()
            #get purchasing from order-up-to levels
            purchasing = min(self.maxInventory, max(0, (self.order_up_to_level[0] - sum(self.inventory_position[a]-sum(issuance_product[p][a] for p in self.products) for a in self.ages)), (self.order_up_to_level[1] - sum(self.inventory_position[a]-sum(issuance_product[p][a] for p in self.products) for a in self.ages[:self.targetAges[0]]))))
        ...
        return observation, norm_reward, False, self.n_steps == self.max_horizon or (self.use_adversarial_sampling and self.n_steps == self.adversarial_threshold), {"revenue":revenue, "purchasing_cost": purchasing_cost, "holding_cost":holding_cost, "decay_salvage":decay_salvage, "outdating_cost": outdating_cost, "purchasing":purchasing, "production":production, "issuance":[sum(issuance_product[p][a] for p in self.products) for a in self.ages], "demand": demand, "outdating": outdating, "overproduction": leftovers, "inventory": starting_inventory, "price":prev_price, "sales": sales, "decay_proportions":decay_proportions, "cdf_vector":cdf_vector, "lp_time":lp_time, "env_time":env_time}
    ...
    def get_newsvendor_purchasing(self):
        #update newsvendor model to current price
        #self.newsvendor_model.update()
        #for p in self.products:
        #     purchase = self.newsvendor_model.getVarByName(f"purchase[{p}]")
        #     purchase.obj = -self.price * self.acc_inv_target[p] - self.acc_c_target[p]
        # self.newsvendor_model.update()
        # #optimize model
        # self.newsvendor_model.optimize()
        # purchasing = sum(self.newsvendor_model.getVarByName(f"purchase[{p}]").X for p in self.products)
        # get purchasing from newsvendor directly and interpolate for current price level
        price_cdf = self.priceDistribution.cdf(self.price)
        #get ratio of price to lb and ub in pre-saved dict self.production_targets_price which contains cdf keys from 0 to 1 with step size 0.001
        idx = np.searchsorted(self.price_levels_sorted, price_cdf) 


        lb = self.price_levels_sorted[idx-1]
        ub = self.price_levels_sorted[idx]
        ratio = (price_cdf-lb)/(ub-lb)
        purchasing = ratio * self.production_targets_price[self.price_levels_sorted[idx]][self.nProducts] + (1-ratio) * self.production_targets_price[self.price_levels_sorted[idx-1]][self.nProducts]

        return purchasing
    
    def get_production_from_newsvendor(self):
        issuance_product = {p: {a: 0 for a in self.ages} for p in self.products}
        if self.newsvendor_prod is None:
            for p in self.products[:-1]:
                self.newsvendor_model_prod[p].update()
                self.newsvendor_model_prod[p].getConstrByName("start_inv").rhs = self.inventory_position[self.targetAges[p]]
                self.newsvendor_model_prod[p].update()
                self.newsvendor_model_prod[p].optimize()
                issuance_product[p][self.targetAges[p]] = sum(self.newsvendor_model_prod[p].getVarByName(f"production[{p},{l}]").X * l for l in self.production_levels[p]) / self.evaporation_remains_per_age_class[self.targetAges[p]]

            issuance_product[self.products[-1]][self.targetAges[self.products[-1]]] = self.inventory_position[self.targetAges[self.products[-1]]] 
        else:
            #get production from newsvendor_prod and interpolate between discrete inventory levels
            for p in self.products[:-1]:
                idx = np.searchsorted(self.inventory_levels_sorted, self.inventory_position[self.targetAges[p]])
                lb = self.inventory_levels_sorted[idx-1]
                ub = self.inventory_levels_sorted[idx]
                ratio = (self.inventory_position[self.targetAges[p]]-lb)/(ub-lb)
                issuance_product[p][self.targetAges[p]] = ratio * self.newsvendor_prod[p][ub] + (1-ratio) * self.newsvendor_prod[p][lb]
            issuance_product[self.products[-1]][self.targetAges[self.products[-1]]] = self.inventory_position[self.targetAges[self.products[-1]]]
        return issuance_product

    def get_production_from_target(self):
        self.production_target_lp.update()
        self.production_target_lp.getConstrByName("outdating").rhs = self.inventory_position[self.numAges-1]
        for a in self.ages:
            self.production_target_lp.getConstrByName(f"inventory_{a}").rhs = self.inventory_position[a]
        self.production_target_lp.update()
        self.production_target_lp.optimize()
        issuance_product = {p: {a: self.production_target_lp.getVarByName(f"issuance[{p},{a}]").X for a in self.ages} for p in self.products}
        return issuance_product

    def conditional_demand_distribution(self, p, gamma):
        return st.norm(loc=self.demand_mean[p] + self.demand_std[p]/self.salesprice_std[p] * self.correlation_demand_salesprice[p]*(gamma - self.salesprice_mean[p]), scale=np.sqrt(1-self.correlation_demand_salesprice[p]**2)*self.demand_std[p])

    def get_critical_ratio(self, p, gamma, price):
        c_u = max(0,gamma*self.evaporation_remains_per_age_class[self.targetAges[p]] - price*self.acc_inv_target[p] - self.acc_c_target[p])
        c_o = price*self.acc_inv_target[p] + self.acc_c_target[p] - self.salvage[p](self.demand_mean[p],gamma) * gamma * self.evaporation_remains_per_age_class[self.targetAges[p]]
        critical_ratio = max(self.min_ppf,min(c_u/(c_u+c_o),self.max_ppf))
        return max(0,self.conditional_demand_distribution(p,gamma).ppf(critical_ratio))

    def get_order_up_to_production_targets(self, p, price=None):
        if price is None:
            price = self.meanPrice
        #get the demand distribution for the product given a certain gamma level
        return sigr.quad(lambda gamma: self.get_critical_ratio(p,gamma,price) * self.salesPriceDistributions[p].pdf(gamma), self.salesPriceDistributions[p].ppf(self.min_ppf), self.salesPriceDistributions[p].ppf(self.max_ppf))[0]
      
    def create_newsvendor_lp(self):
        self.newsvendor_model = gb.Model()
        #disable printout of Gurobi solution process
        self.newsvendor_model.setParam('OutputFlag', 0)
        self.newsvendor_model.setParam('Method', 1)

        #decision variables for production
        prod_indices = [(p,l) for p in self.products for l in self.production_levels[p]]
        production = self.newsvendor_model.addVars(prod_indices, lb=0.0, name="production")
        #decision variables for purchasing with lb 0 and ub maxInventory
        purchase = self.newsvendor_model.addVars(self.products, lb=0.0, ub=self.maxInventory, name="purchase")

        #add constraints
        for p in self.products:
            #production restrictions
            self.newsvendor_model.addLConstr(gb.quicksum(production[p,l] for l in self.production_levels[p]) <= 1)
            #purchasing/production relation considering evaporation losses
            self.newsvendor_model.addLConstr(gb.quicksum(production[p,l] * l for l in self.production_levels[p]) <= purchase[p] * self.evaporation_remains_per_age_class[self.targetAges[p]]/self.acc_inv_target[p])
        #purchase restrictions
        self.newsvendor_model.addLConstr(gb.quicksum(purchase[p] for p in self.products) <= self.maxInventory)

        #formulate objective function as piecewise linear function
        obj = gb.quicksum(production[p,l] * self.expected_revenue[p][l] for p in self.products for l in self.production_levels[p]) - \
              gb.quicksum(purchase[p] * (self.price * self.acc_inv_target[p] + self.acc_c_target[p]) for p in self.products)
        
        self.newsvendor_model.setObjective(obj, GRB.MAXIMIZE)

        #create a model distributing the remaining inventory in target ages to the products
        self.newsvendor_model_prod = {p: gb.Model() for p in self.products[:-1]}

        for p in self.products[:-1]:
            self.newsvendor_model_prod[p].setParam('OutputFlag', 0)
            self.newsvendor_model_prod[p].setParam('Method', 1)
            #decision variables for production
            prod_indices = [(pp,l) for pp in self.products[p:] for l in self.production_levels[pp]]
            production = self.newsvendor_model_prod[p].addVars(prod_indices, name="production")
            #add constraints
            #production restrictions
            for pp in self.products[p:]: 
                self.newsvendor_model_prod[p].addLConstr(gb.quicksum(production[pp,l] for l in self.production_levels[pp]) <= 1)

            self.newsvendor_model_prod[p].addLConstr(gb.quicksum(production[pp,l] * l / (np.prod([1-self.meanDecay[a] for a in self.ages[self.targetAges[p]+1:self.targetAges[pp]+1]]) * self.evaporation_remains_per_age_class[self.targetAges[pp]]) for pp in self.products[p:] for l in self.production_levels[pp]) <= self.inventory_position[self.targetAges[p]], name="start_inv")
            #formulate objective function as piecewise linear function
            obj = gb.quicksum(production[pp,l] * (self.expected_revenue[pp][l] - l*sum((self.holdingCosts-self.decaySalvage[a]*self.meanDecay[a])*np.prod([1-self.meanDecay[j] for j in self.ages[self.targetAges[p]+1:a+1]])*(1/self.evaporation_remains_per_age_class[a]) for a in self.ages[self.targetAges[p]+1:self.targetAges[pp]+1])) for pp in self.products[p:] for l in self.production_levels[pp]) 
            self.newsvendor_model_prod[p].setObjective(obj, GRB.MAXIMIZE)

    def create_production_target_lp(self):
        self.production_target_lp = gb.Model()
        #disable printout of Gurobi solution process
        self.production_target_lp.setParam('OutputFlag', 0)
        self.production_target_lp.setParam('Method', 1)

        #decision variables for production
        production = self.production_target_lp.addVars(self.products, lb=0.0, ub=self.maxInventory, name="production")
        issuance = self.production_target_lp.addVars(self.products, self.ages, name="issuance")

        target_age_excess = self.production_target_lp.addVars(self.products, lb=0.0, name="target_age_excess")
        production_mismatch_pos = self.production_target_lp.addVars(self.products, lb=0.0, name="production_mismatch_pos")
        production_mismatch_neg = self.production_target_lp.addVars(self.products, lb=0.0, name="production_mismatch_neg")

        #add constraints
        for p in self.products:
            #relate issuance to production
            self.production_target_lp.addLConstr(gb.quicksum(issuance[p,a] * self.evaporation_remains_per_age_class[a] for a in self.ages) == production[p])
            #target age adherence
            self.production_target_lp.addLConstr(gb.quicksum(issuance[p,a] * a * self.evaporation_remains_per_age_class[a] for a in self.ages) - target_age_excess[p] == self.targetAges[p] * gb.quicksum(issuance[p,a] * self.evaporation_remains_per_age_class[a] for a in self.ages))

        #avoid outdating
        self.production_target_lp.addLConstr(gb.quicksum(issuance[p,self.numAges-1] for p in self.products) >= self.inventory_position[self.numAges-1], name="outdating")

        #constraints for issuance by inventory
        for a in self.ages:
            self.production_target_lp.addLConstr(gb.quicksum(issuance[p,a] for p in self.products) <= self.inventory_position[a], name="inventory_"+str(a))

        #constraints for production mismatch
        for p in self.products:
            self.production_target_lp.addLConstr(production[p] - self.production_targets[p] == production_mismatch_pos[p] - production_mismatch_neg[p])

        #set objective value which penalizes production mismatches and target age excesses
        obj = gb.quicksum((production_mismatch_pos[p] + production_mismatch_neg[p]) * self.salesprice_mean[p] for p in self.products) + gb.quicksum(target_age_excess[p] for p in self.products)
        self.production_target_lp.setObjective(obj, GRB.MINIMIZE)
    ...
    def set_heuristic(self, heuristic):
        ...
        if heuristic == "LP":
            self.simulate_heuristic = True
            self.use_newsvendor = False
            self.use_order_up_to = False
            self.use_volume_target = False
        elif heuristic == "NVP+NVt":
            self.simulate_heuristic = False
            self.use_newsvendor = True
            self.use_order_up_to = False
            self.use_volume_target = False
        elif heuristic == "2DO+NVt":
            self.simulate_heuristic = False
            self.use_newsvendor = False
            self.use_order_up_to = True
            self.use_volume_target = False
        elif heuristic == "NVP+VOL":
            self.simulate_heuristic = False
            self.use_newsvendor = True
            self.use_order_up_to = False
            self.use_volume_target = True
        elif heuristic == "2DO+VOL":
            self.simulate_heuristic = False
            self.use_newsvendor = False
            self.use_order_up_to = True
            self.use_volume_target = True
    ...
    def simulate_n_steps(self, nsteps=1, policy=None, plot=False, warm_up=100, heuristic=None):
    ...
    def simulate_record_times(self, policy=None, n_steps = 100):
        if policy == None:
            raise ValueError("Policy must be provided for time recording")
        self.n_steps = 0
        times = {"lp_time": np.array([]), "env_time": np.array([]), "nn_time": np.array([])}
        while(self.n_steps < n_steps):
            start = time.time()
            action = policy.compute_single_action(self._get_obs(), explore=False)
            times["nn_time"] = np.append(times["nn_time"], time.time()-start)
            next_state, reward, truncated, done, info = self.step(action)
            times["env_time"] = np.append(times["env_time"], info["env_time"])
            times["lp_time"] = np.append(times["lp_time"], info["lp_time"])

        return times
    
    def evaluate_drl(self, seeds, policy=None, starting_price=None, starting_inv=None, replications=1000, do_heuristic=False):
        if policy==None:
    ...
    def evaluate_heuristic(self, seeds, starting_price=None, starting_inv=None, replications=1000, heuristic="LP"):
        #create cdfs using seeds (matrix size depends on replications)
        cdfs = {}
        ...
        print("EVALUATING HEURISTIC: ", heuristic)
        ...
        for seed in seeds:
            #np.random.seed(seed)
            print(f"EVALUATING HEURISTIC {heuristic} WITH SEED: ", seed)
            rewards_heuristic = np.append(rewards_heuristic, [self.simulate_w_cdfs(cdfs=cdfs[seed], policy=None, initial_price=starting_price, initial_inventory=starting_inv, heuristic=heuristic)[1]])

        print(f"HEURISTIC {heuristic} average reward: ", np.mean(rewards_heuristic))
        
        return rewards_heuristic
    ...
    def simulate_w_cdfs(self, cdfs=None, policy=None, initial_price = None, initial_inventory=None, heuristic=None):
